...
const SIPCalculator = () => {
  ...
  const getExportConfig = (): ExportConfig => {
    const wealthGain = result.totalInvested > 0
      ? `${((result.totalReturns / result.totalInvested) * 100).toFixed(1)}%`
      : '0%';

    return {
      title: 'SIP Calculator Report',
      fileNamePrefix: 'SIP_Report',
      sections: [
        {
          title: 'Investment Parameters',
          keyValues: [
            ['Monthly Investment', formatCurrencyFull(parseFloat(monthlyInvestment) || 0)],
        ...
        },
        {
          ...
          keyValues: [
            ['Total Invested', formatCurrencyFull(result.totalInvested)],
            ['Total Returns', `+${formatCurrencyFull(result.totalReturns)}`],
            ['Maturity Amount', formatCurrencyFull(result.maturityAmount)],
            ['Wealth Gain', wealthGain],
          ],
        },
        {
          title: 'Formula',
