...
import { ArrowLeft, Plus, Trash2, TrendingUp, TrendingDown, PieChart, BarChart3, IndianRupee, Percent, AlertTriangle, Download } from "lucide-react";
...
const StockPortfolioTracker = () => {
  ...
  const exportToPDF = () => {
    ...
    doc.text('Stock Portfolio Report', pageWidth / 2, 18, { align: 'center' });
    doc.setFontSize(10);
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['Returns', `${totalPnLPercent >= 0 ? '+' : ''}${totalPnLPercent.toFixed(2)}%`],
      ],
      theme: 'grid',
      styles: { fontSize: 11 },
    ...
    let currentY = (doc as any).lastAutoTable?.finalY + 10 || 90;

    autoTable(doc, {
      startY: currentY,
      head: [['Stock', 'Qty', 'Buy Price', 'Current', 'P&L', '%', 'Sector']],
      body: holdingsWithMetrics.map(h => [
        h.name, h.quantity, formatCurrency(h.buyPrice), formatCurrency(h.currentPrice),
        `${h.pnl >= 0 ? '+' : ''}${formatCurrency(h.pnl)}`, `${h.pnlPercent.toFixed(1)}%`, h.sector
      ]),
      theme: 'striped',
